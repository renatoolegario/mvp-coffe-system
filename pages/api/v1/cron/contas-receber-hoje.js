...
import { runContasReceberReminderJob } from "../../../../services/reminders";
...
export default async function handler(req, res) {
  ...
  if (!isVercelCronRequest(req)) {
    ...
    if (!auth) {
      await safeRegisterAuditLog({
        type: "cron",
        key: "contas-receber-hoje",
        description: "Cron de contas a receber do dia",
        endpoint: "/api/v1/cron/contas-receber-hoje",
        method: "GET",
        origin,
        status: "NAO_AUTORIZADO",
        httpStatus: res.statusCode || 401,
        event: "contas_receber_hoje",
        reference: "contas_receber_hoje",
  ...
  try {
    const result = await runContasReceberReminderJob();
    await safeRegisterAuditLog({
      type: "cron",
      key: "contas-receber-hoje",
      description: "Cron de contas a receber do dia",
      ...
      httpStatus: 200,
      event: result?.categoria || "contas_receber_hoje",
      reference: result?.categoria || "contas_receber_hoje",
      requestHeaders: req.headers,
      requestPayload: { query: req.query || {} },
    ...
    await safeRegisterAuditLog({
      ...
      responsePayload: {},
      metadata: buildAuditMetadata(req, auth),
      error: error.message || "Erro ao executar cron de contas a receber.",
    });
