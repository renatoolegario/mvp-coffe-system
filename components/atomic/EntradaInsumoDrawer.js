...
export default function EntradaInsumoDrawer({ open, onClose }) {
  ...
  useEffect(() => {
    ...
    const unidadeDefault = String(
      insumoSelecionado.unidade_codigo || "KG",
    ).toUpperCase();
            ...
            {modoEntrada === "KG" ? (
              ...
              <>
                <TextField
                  ...
                  onChange={(event) => setKgPorSacoEntrada(event.target.value)}
                  inputProps={{ min: 0, step: "0.01" }}
                  required
                />
                <TextField
                  ...
                  onChange={(event) => setQuantidadeSacos(event.target.value)}
                  inputProps={{ min: 0, step: "0.01" }}
                  required
                />
              </>
                  ...
                  <Grid container spacing={1} key={`parcela-${parcelaNum}`}>
                    ...
                    <Grid item xs={12} sm={4}>
                      <SearchableSelect
                        ...
                        onChange={(event) =>
                          handleChangeStatusParcela(index, event.target.value)
                        }
                        fullWidth
                        required
                      >
                        <MenuItem value="A_PRAZO">A prazo (não pago)</MenuItem>
                ...
                return (
                  <Paper key={item.id} variant="outlined" sx={{ p: 2 }}>
                    <Stack spacing={1.5}>
                      ...
                      >
                        Soma das parcelas: {formatCurrency(totalParcelasExtra)}
                        {" • "}
                      ...
                      {parcelasDivergentesExtra ? (
                        <Alert severity="warning">
                          Ajuste os valores das parcelas para fechar o total do
                          custo extra.
                        </Alert>
                      ) : null}
                            ...
                            >
                              ...
                              <Grid item xs={12} sm={4}>
                                <TextField
                                  ...
                                  type="date"
                                  value={
                                    item.parcelas_vencimentos[parcelaIndex] ||
                                    ""
                                  }
                                  onChange={(event) =>
